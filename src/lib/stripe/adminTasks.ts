...
export const copyBillingDetailsToCustomer = async (
  ...
  try {
    await db.update(users).set({
      billingAddress: { ...address }, paymentMethod: { ...payment_method[payment_method.type] },
...
export const manageSubscriptionStatusChange = async (
  ...
  try {
    const customerData = await db.query.customers.findFirst({ where: (c, { eq }) => eq(c.stripeCustomerId, customerId) })
    if (!customerData) throw new Error('Could not find customer');
