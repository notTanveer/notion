...
import { Divide, Router, XCircleIcon } from "lucide-react";
...
const QuillEditor: React.FC<QuillEditorProps> = ({
  ...
  const iconOnChange = async (icon: string) => {
    if (!fileId) return;
    if (dirType === "file") {
      if (!folderId || !workspaceId) return; // we need workspace and folder id, as we are looking for a file.
      dispatch({
        type: "UPDATE_FILE",
        payload: { file: { iconId: icon }, fileId, folderId, workspaceId },
      });
  ...
  useEffect(() => {
    ...
    const fetchInformation = async () => {
      if (dirType === "file") {
        ...
        quill.setContents(selectedDir[0].data || "");
        dispatch({
          type: "UPDATE_FILE",
          payload: {
            file: selectedDir[0],
      ...
      if (dirType === "workspace") {
        ...
        if (quill === null || !selectedDir[0]) return;
        if (!selectedDir[0].data) return;
        quill.setContents(JSON.parse(selectedDir[0].data || ""));
        dispatch({
          type: "UPDATE_WORKSPACE",
  ...
  }, [quill, socket, fileId, user, details, workspaceId]);

  useEffect(() => {
    if (socket === null || !fileId || quill === null) return;
  ...
  return (
    <>
      <div className="relative">
        ...
        <div className="flex flex-col-reverse sm:flex-row sm:justify-center sm:items-center sm:p-2 p-8">
          <div>{breadCrumbs}</div>
